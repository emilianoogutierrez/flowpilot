...
function loginScreen() {
    ...
    const errorMessage = el('p', 'form-error');
    ...
    form.addEventListener('submit', event => {
        ...
        busy(submit, async () => {
            try {
                user = await post('/auth/login', { email: email.value, password: password.value });
                password.value = '';
                selectWorkspace();
                mountShell();
                await renderRoute();
...
function selectWorkspace() {
    if (!user?.workspaces.length)
        throw new Error('This account has no workspace. Ask the operator to add a membership.');
    if (!user.workspaces.some(workspace => workspace.id === getWorkspace()))
        setWorkspace(user.workspaces[0].id);
}
function mountShell() {
    if (!user)
        return;
    ...
    choice.classList.add('workspace-select');
    choice.setAttribute('aria-label', 'Workspace');
    ...
    for (const item of navigation) {
        ...
        anchor.dataset.route = item.path;
        nav.append(anchor);
    }
    ...
    const reference = el('a', 'api-link', icon('code'), 'API schema');
    reference.href = '/api/openapi.json';
    ...
    currentPage = el('main', 'main-content');
    currentPage.id = 'main';
...
async function renderRoute(quiet = false) {
    ...
    const openSteps = new Set([...pageElement.querySelectorAll('details[open]')].map(item => item.dataset.step));
    for (const anchor of document.querySelectorAll('.nav-link')) {
    ...
    if (currentBreadcrumb)
        currentBreadcrumb.textContent = navigation.find(item => item.path === section)?.label ?? 'Overview';
    if (!quiet)
        pageElement.replaceChildren(el('div', 'loading', el('span', 'spinner'), 'Loading workspace…'));
    document.title = `${navigation.find(item => item.path === section)?.label ?? 'Overview'} · FlowPilot`;
    try {
        let content;
        if (section === 'workflows' && path[1])
            content = await workflowPage(path[1], user);
        else if (section === 'workflows')
            content = await workflowsPage(user);
        else if (section === 'runs' && path[1])
            content = await runPage(path[1], user);
        else if (section === 'runs')
            content = await runsPage();
        else if (section === 'credentials')
            content = await credentialsPage(user);
        else if (section === 'workspace')
            content = await workspacePage(user);
        else
            content = await overviewPage(user);
        if (ticket !== generation)
            return;
        if (quiet && mountedHash === route) {
            for (const details of content.querySelectorAll('details'))
                if (openSteps.has(details.dataset.step))
                    details.open = true;
        }
...
setInterval(() => {
    ...
    if (user && !document.hidden && !formFocused && !document.querySelector('dialog[open]') && (route.includes('/runs') || route.includes('/overview') || !route))
