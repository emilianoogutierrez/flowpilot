...
import type { Collection, Credential, Definition, Overview, Project, Run, RunDetail, User, Workflow, WorkspaceDetail } from './types.js';
...
function heading(eyebrow: string, title: string, subtitle: string, ...actions: HTMLElement[]): HTMLElement {
...
function panel(title: string, body: HTMLElement, trailing?: HTMLElement): HTMLElement {
  return el('section', 'panel', el('div', 'panel-heading', el('h2', '', title), trailing), body);
}

function link(text: string, path: string, className = 'text-link'): HTMLAnchorElement {
...
export async function newWorkflow(): Promise<void> {
  const projects = await api<{ items: Project[] }>('/projects');
  ...
  const submit = button('Create workflow', async () => {
    await busy(submit, async () => {
      const item = templates[Number(template.value)]!;
      const result = await post<Workflow & { webhook_secret: string }>('/workflows', { name: name.value, project_id: project.value, description: item.description, definition: item.definition });
      dialog.close();
...
function revealSecret(title: string, value: string, description: string): void {
...
export function runTable(items: Run[]): HTMLElement {
  if (!items.length) return empty('No executions yet', 'Publish a workflow, then send its first event.');
  ...
  for (const run of items) {
    ...
    body.append(el('tr', '',
      el('td', '', link(run.id.slice(0, 8), `runs/${run.id}`, 'mono table-link')),
      el('td', '', el('strong', 'table-name', run.workflow_name || 'Workflow'), el('span', 'row-subtitle', `Version ${run.version ?? '—'}`)),
      el('td', '', badge(run.status)),
      el('td', '', el('span', 'subtle-tag', run.dry_run ? 'Dry run' : label(run.source))),
      el('td', 'mono muted', duration(elapsed)),
      el('td', 'muted nowrap', timestamp(run.created_at)),
    ));
  }
  table.append(body);
...
function activityChart(data: Overview['activity']): HTMLElement {
  ...
  for (const day of data) {
    ...
    if (day.total === 0) total.classList.add('zero');
...
export async function overviewPage(user: User): Promise<HTMLElement> {
  ...
  const stat = (title: string, value: string, note: string, symbol: string, accent = false) => el('section', `stat-card ${accent ? 'accent' : ''}`, el('div', 'stat-label', title, icon(symbol)), el('strong', 'stat-value', value), el('span', 'stat-note', note));
  page.append(el('div', 'stats-grid',
    stat('Workflows', String(data.workflow_count), 'Across this workspace', 'workflow'),
    stat('Executions', String(data.total), `${data.dry_runs} dry runs included`, 'runs'),
    stat('Success rate', data.success_rate === null ? '—' : `${data.success_rate}%`, 'Succeeded ÷ (succeeded + failed)', 'check', true),
    stat('In progress', String(pending), 'Queued, running or waiting', 'clock'),
  ));
  ...
  for (const run of waiting.slice(0, 2)) attention.append(link(run.workflow_name + ' →', `runs/${run.id}`, 'attention-link'));
  if (!waiting.length) attention.append(link('View executions →', 'runs', 'attention-link'));
...
export async function workflowsPage(user: User): Promise<HTMLElement> {
  const data = await api<Collection<Workflow>>('/workflows?limit=100');
  const create = button('New workflow', newWorkflow, 'button primary', 'plus');
  create.disabled = !writable(user);
  ...
  const render = () => {
    ...
    if (!matches.length) content.append(empty('No matching workflows', data.total ? 'Try another search.' : 'Create a workflow to get started.'));
  };
...
function sampleInput(definition: Definition): Record<string, unknown> {
  const properties = definition.input_schema.properties as Record<string, { type?: string }> | undefined;
  const defaults: Record<string, unknown> = { company: 'Example Studio', message: 'We need help connecting an API to our app.', budget: 12000, amount: 1500, name: 'Example event' };
...
async function startRun(workflow: Workflow): Promise<void> {
  ...
  const submit = button('Start execution', async () => {
    await busy(submit, async () => {
      const result = await post<Run>(`/workflows/${workflow.id}/runs`, { input: JSON.parse(payload.value) as unknown, dry_run: dry.checked }, { 'Idempotency-Key': crypto.randomUUID() });
      dialog.close();
...
export async function workflowPage(id: string, user: User): Promise<HTMLElement> {
  let workflow = await api<Workflow>(`/workflows/${id}`);
  ...
  concurrent.min = '1'; concurrent.max = '16';
  ...
  const validate = button('Validate', async () => {
    const definition = JSON.parse(editor.value) as Definition;
    await post('/definitions/validate', definition);
    graphArea.replaceChildren(graph(definition.nodes));
    toast('Definition is valid');
  }, 'button quiet', 'check');
  const saveChanges = async () => {
    const result = await api<Workflow>(`/workflows/${id}`, { method: 'PATCH', body: JSON.stringify({ definition: JSON.parse(editor.value), expected_revision: workflow.draft_revision, name: name.value, description: description.value, max_concurrency: Number(concurrent.value) }) });
    workflow = { ...workflow, ...result, definition: JSON.parse(editor.value) as Definition };
  ...
  const publish = button('Publish version', async () => busy(publish, async () => {
    await saveChanges();
    const result = await post<{ number: number }>(`/workflows/${id}/publish`, { expected_revision: workflow.draft_revision });
  ...
  const download = button('Export JSON', () => {
    ...
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'workflow.json'; anchor.click(); URL.revokeObjectURL(url);
  }, 'button quiet', 'code');
  ...
  for (const version of workflow.versions) {
    const activate = button(version.number === workflow.published_version ? 'Active' : 'Activate', async () => {
      ...
      toast(`Version ${version.number} activated`); window.dispatchEvent(new Event('flowpilot:refresh'));
    }, 'button small-button');
  ...
  if (!workflow.versions.length) versionRows.append(empty('No published versions', 'Publishing creates an immutable snapshot.'));
  const rotate = button('Rotate signing key', async () => {
    if (!confirm('Rotate the webhook key? Existing senders will need the new key.')) return;
    const result = await post<{ webhook_secret: string }>(`/workflows/${id}/rotate-webhook`);
...
export async function runsPage(): Promise<HTMLElement> {
  ...
  const data = await api<Collection<Run>>(`/runs?limit=20&offset=${pageNumber * 20}&status=${encodeURIComponent(status)}`);
...
async function replay(run: RunDetail): Promise<void> {
  const dry = input('', 'checkbox'); dry.checked = true;
  ...
  const submit = button('Create replay', async () => busy(submit, async () => {
    const result = await post<Run>(`/runs/${run.id}/replay`, { dry_run: dry.checked, acknowledge_side_effects: acknowledged.checked });
    dialog.close(); navigate(`runs/${result.id}`);
  }), 'button primary');
  const dialog = modal('Replay execution', content, [submit]);
}

export async function runPage(id: string, user: User): Promise<HTMLElement> {
  const run = await api<RunDetail>(`/runs/${id}`);
  const isClosed = ['succeeded', 'failed', 'cancelled'].includes(run.status);
  const cancel = button('Cancel execution', async () => {
    if (!confirm('Cancel this execution? An external request already in flight cannot be undone.')) return;
    await post(`/runs/${id}/cancel`); window.dispatchEvent(new Event('flowpilot:refresh'));
  }, 'button danger', 'close');
  ...
  if (run.error_code) page.append(el('div', 'callout error-callout', icon('info'), el('span', '', `Execution stopped: ${run.error_code}`)));
  ...
  for (const [index, step] of run.steps.entries()) {
    ...
    if (step.status === 'awaiting_approval') {
      const note = input(); note.placeholder = 'Optional review note'; note.setAttribute('aria-label', 'Approval note');
    ...
    if (step.error_code) body.append(el('p', 'error-text', step.error_code));
    if (step.ready_at && ['waiting', 'retry_wait', 'awaiting_approval'].includes(step.status)) body.append(el('p', 'small muted', `${step.status === 'awaiting_approval' ? 'Approval deadline' : 'Next eligible time'}: ${timestamp(step.ready_at)}`));
    body.append(el('h4', '', 'Output'), jsonView(step.output));
    if (Object.keys(step.usage).length) body.append(el('h4', '', 'Provider usage'), jsonView(step.usage));
    if (step.attempts.length) {
      body.append(el('h4', '', 'Attempts'));
      for (const attempt of step.attempts) body.append(el('div', 'attempt-row', el('span', 'mono', `#${attempt.number}`), badge(attempt.status), el('span', 'small muted', timestamp(attempt.started_at)), el('span', 'small mono', duration(attempt.duration_ms)), el('code', 'small error-text', attempt.error_code ?? '')));
    }
  ...
  if (run.parent_id) info.append(el('span', 'muted', 'Replay of'), link(run.parent_id.slice(0, 8), `runs/${run.parent_id}`));
  page.append(el('div', 'two-panels', inputPanel, panel('Metadata', info)));
  return page;
}

async function credentialDialog(): Promise<void> {
  const projects = await api<{ items: Project[] }>('/projects');
  const name = input();
  ...
  const host = input(); host.placeholder = 'api.example.com';
  const value = input('', 'password'); value.autocomplete = 'new-password';
  ...
  const submit = button('Save credential', async () => busy(submit, async () => {
    ...
    value.value = ''; dialog.close(); toast('Credential saved'); window.dispatchEvent(new Event('flowpilot:refresh'));
  }), 'button primary', 'key');
  const dialog = modal('Add credential', content, [submit]);
}

export async function credentialsPage(user: User): Promise<HTMLElement> {
  const data = await api<{ items: Credential[] }>('/credentials');
  const create = button('Add credential', credentialDialog, 'button primary', 'plus'); create.disabled = !admin(user);
  ...
  for (const credential of data.items) {
    const rotate = button('Rotate', async () => {
      ...
      const submit = button('Save new version', async () => busy(submit, async () => {
        await post(`/credentials/${credential.id}/rotate`, { value: value.value }); value.value = ''; dialog.close(); window.dispatchEvent(new Event('flowpilot:refresh'));
      }), 'button primary');
    ...
    const revoke = button('Revoke', async () => {
      if (!confirm('Revoke this credential? Pending steps using it will fail.')) return;
      await post(`/credentials/${credential.id}/revoke`); window.dispatchEvent(new Event('flowpilot:refresh'));
...
export async function workspacePage(user: User): Promise<HTMLElement> {
  const data = await api<WorkspaceDetail>('/workspace');
  const projects = await api<{ items: Project[] }>('/projects');
  ...
  for (const member of data.members) members.append(el('div', 'member-row', el('div', 'avatar', initials(member.name)), el('div', 'member-info', el('strong', '', member.name), el('span', 'muted small', member.email)), el('span', 'subtle-tag', label(member.role))));
  const addMember = button('Add member', () => {
    ...
    const submit = button('Save membership', async () => {
      await post('/members', { email: email.value, role: role.value }); dialog.close(); window.dispatchEvent(new Event('flowpilot:refresh'));
    }, 'button primary');
  ...
  for (const project of projects.items) projectList.append(el('div', 'version-row', el('strong', '', project.name), el('code', 'small muted', project.id)));
  ...
  for (const event of data.events) events.append(el('div', 'audit-row', el('div', 'audit-marker'), el('strong', 'mono small', event.action), el('code', 'muted small', event.resource_id.slice(0, 8)), el('time', 'muted small', timestamp(event.created_at))));
